...
class Program
{
    ...
    {
        await Parser.Default.ParseArguments<Options>(args).WithParsedAsync<Options>(RunBenchmark);
    ...
    static async Task RunSqlServerBenchmark(Options opts)
    {
        ...
        var createTableCmd = new SqlCommand(
            "CREATE TABLE FinancialRecords (Id INT PRIMARY KEY IDENTITY, Amount DECIMAL(18, 2), Date DATETIME, Description NVARCHAR(255))",
            connection
        );
        await createTableCmd.ExecuteNonQueryAsync();
        await new SqlCommand(
            "TRUNCATE TABLE ProcessedFinancialRecords",
            connection
            ...
            tasks.Add(
                Task.Run(async () =>
                {
                    ...
                    for (int j = 0; j < opts.Rows / opts.Threads; j += opts.BatchSize)
                    {
                        ...
                        var cmdText =
                            "INSERT INTO FinancialRecords (Amount, Date, Description) VALUES ";

                        for (int k = 0; k < opts.BatchSize; k++)
                        {
                            cmdText +=
                                $"({random.Next(1, 1000)}, '{DateTime.Now}', 'Sample Data {k}'),";
                        }
        ...
        Console.WriteLine(
            $"SQL Server: Inserted {opts.Rows} rows in {stopwatch.ElapsedMilliseconds} ms"
        );
        ...
        var createProcessedTableCmd = new SqlCommand(
            "CREATE TABLE ProcessedFinancialRecords (Id INT PRIMARY KEY IDENTITY, Amount DECIMAL(18, 2), Date DATETIME, Description NVARCHAR(255), ProcessedDate DATETIME)",
            connection
        );
        await createProcessedTableCmd.ExecuteNonQueryAsync();

        stopwatch.Restart();
        await ProcessRecordsAsync(opts.BatchSize, connectionString);
        stopwatch.Stop();

        Console.WriteLine(
            $"SQL Server: Processed {opts.Rows} rows in {stopwatch.ElapsedMilliseconds} ms"
        );
    ...
    )
    {
        ...
        var tasks = batches
            .Select(batch => Task.Run(() => ProcessBatchAsync(batch, connectionString)))
            .ToArray();

        await Task.WhenAll(tasks);
    }

    private static async Task<List<FinancialRecord>> GetFinancialRecordsAsync(
    ...
    private static List<List<FinancialRecord>> SplitIntoBatches(
    ...
    )
    {
        ...
        await connection.OpenAsync();
        var insertQuery =
            "INSERT INTO ProcessedFinancialRecords (Amount, Date, Description, ProcessedDate) VALUES (@Amount, @Date, @Description, GETDATE())";
        ...
        using (var transaction = connection.BeginTransaction())
        {
            using (var cmd = new SqlCommand(insertQuery, connection, transaction))
            {
                ...
                cmd.Parameters.Add(
                    new SqlParameter("@Description", System.Data.SqlDbType.NVarChar, 255)
                );
    ...
    static async Task RunRedisBenchmark(Options opts)
    {
        ...
        Console.WriteLine(
            $"Redis: Inserted {opts.Rows} rows in {stopwatch.ElapsedMilliseconds} ms"
        );
            ...
            tasks.Add(
                Task.Run(async () =>
                {
                    ...
                    await db.HashSetAsync(
                        $"ProcessedFinancialRecords:{id}",
                        entries
                            .Append(new HashEntry("ProcessedDate", DateTime.Now.ToString()))
                            .ToArray()
                    );
                })
            );
        ...
        Console.WriteLine(
            $"Redis: Processed {opts.Rows} rows in {stopwatch.ElapsedMilliseconds} ms"
        );

        // Calculate the number of random accesses to perform
        int numberOfRandomAccesses = processedKeys.Length / 10;
        if (numberOfRandomAccesses == 0 && processedKeys.Length > 0)
        {
            numberOfRandomAccesses = 1; // Ensure at least one access if there are processed records
        }

        Console.WriteLine($"Performing {numberOfRandomAccesses} random accesses.");

        // Perform random row access and measure time
        stopwatch.Restart();
        var random = new Random();
        var randomAccessTasks = new List<Task>();

        for (int i = 0; i < numberOfRandomAccesses; i++)
        {
            var randomKey = processedKeys[random.Next(processedKeys.Length)];
            randomAccessTasks.Add(db.HashGetAllAsync(randomKey));
        }

        Task.WhenAll(randomAccessTasks).Wait();
        stopwatch.Stop();

        Console.WriteLine($"Time spent on random row access: {stopwatch.ElapsedMilliseconds} ms");
    }
}
